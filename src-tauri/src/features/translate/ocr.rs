...
async fn recognize_with_baidu(
    ...
    let (api_key, secret_key) =
        super::baidu_creds::resolve_baidu_ocr_credentials(ocr_api_key, ocr_secret_key);
...
fn baidu_ocr_ready(api_key: &str, secret_key: &str) -> bool {
    let (k, s) = super::baidu_creds::resolve_baidu_ocr_credentials(api_key, secret_key);
...
async fn recognize_with_youdao(
    ...
    let app_key = super::youdao::resolve_youdao_app_key(app_key_cfg);
    let app_secret = super::youdao::resolve_youdao_app_secret(secret_cfg);
    ...
    let (salt, curtime, sign) = super::youdao::sign_v3(&app_key, &app_secret, &b64);
    ...
    let response = client
        .post(super::youdao::YOUDAO_OCR_URL)
        .form(&params)
        .send()
        .await

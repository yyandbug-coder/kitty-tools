//! 有道智云 OpenAPI：文本翻译 `https://openapi.youdao.com/api`、通用 OCR `https://openapi.youdao.com/ocrapi`
//! 签名 signType=v3：`sign = sha256(appKey + input + salt + curtime + appSecret)`，input 为 q/img 的截断规则。
...
/// 与官方一致：长度按 **Unicode 字符数** 计（≤20 用全文，否则前10 + len + 后 10）。

...
1、系统中的提示统一使用 react-hot-toast ，不要使�?alert

2、所组件必须使用 shadcn/ui 库中的组件，不要使用其他的组件库，不要自己写组件，像 input，button等也不要使用原生组件，除�?shadcn/ui 中没有提供相关的组件，如�?shadcn/ui 中没有提供相关组件，可以使用 tailwind css 来实现样式，但组件的交互逻辑必须自己实现，不能使用其他的组件库来实现交互逻辑，组件的样式也要尽量�?shadcn/ui 中的组件保持一致，不要有太大的差异

3、需要严格保�?pc 和移动端都套兼容

4、确保所有的功能要支�?windows、macos，并且要响应式设计，适配不同的屏幕尺寸和分辨�?

5、日期处理统一使用 dayjs 库，不要使用其他的日期处理库，也不要使用原生�?Date 对象

6、每个页面头部都需要添加注释，说明该页面的功能和用�?

7、系统导出文件的功能需要同步支�?windows、macos

8、components 目录下除�?ui，其他组件格式必须为xxx/index.tsx
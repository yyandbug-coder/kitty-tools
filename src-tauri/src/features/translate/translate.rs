...
impl TranslateCreds {
    pub fn from_config(cfg: &super::config::AppConfig) -> Self {
        Self {
...
fn effective_source_lang(request: &TranslateRequest) -> String {
    ...
    super::lang_detect::detect_source_for_auto(&request.text)
        .map(|s| s.to_string())
...
async fn baidu_translate(
    ...
    let (app_id, secret) = super::baidu_creds::resolve_baidu_credentials(app_id, secret);
...
async fn youdao_translate(
    ...
    let app_key = super::youdao::resolve_youdao_app_key(&creds.youdao_app_key);
    let app_secret = super::youdao::resolve_youdao_app_secret(&creds.youdao_app_secret);
    ...
    let (salt, curtime, sign) =
        super::youdao::sign_v3(&app_key, &app_secret, request.text.as_str());
    ...
    let response = client
        .post(super::youdao::YOUDAO_TRANSLATE_URL)
        .form(&params)
        .send()
        .await
